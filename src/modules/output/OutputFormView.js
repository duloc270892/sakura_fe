...
class OutputFormView extends React.Component {
    ...
    _validateSave() {
        ...
        ListProductByOrderOutputGroup.map((item) => {
            ...
            //     return
            // }
            if (parseInt(NumberTemp) + parseInt(ProductsEachOutput) > Number) {
    ...
    ChangeButton(value) {
        let valueTemp = value
        let isStatus = true
        let { objDataOutput, listProductsSelected, ListProductByOrderOutput, isLoadingOutput } = this.props.output
        let { status } = this.props.toolbar
        if (["SAVE", "SAVEANDCLOSE"].indexOf(valueTemp) != -1 && isLoadingOutput) {
            this.child._addNotification(`Dữ liệu sản phẩm đang tải, vui lòng đợi ...`, 'warning')
            return
        } else {
            ...
            switch (valueTemp) {
                case "SAVEANDCLOSE":
                    ...
                    // isStatus = false
                    if (status == 'EDIT') {
                        this.props.updateAddListProducts(ListProduct_temp_new).then(res => {
                            let { data } = res.data
                            if (data && data.ErrorMessage) {
                                alert('Lỗi trong quá trình lưu dữ liệu.')
                            } else {
                                this.props.updateItemOutput(ListProduct_temp_new).then(() => {
                                    this.child._addNotification(`Cập nhật thành công!!!`, 'success')
                                    // $(`#toolbar_${valueTemp}`).css('pointer-events', 'auto');
                                    //Cap nhat lai trong luong san pham bo
                                    const { objDataOutput } = this.props.output
                                    this.props.UpdateProductWeightReal(objDataOutput.IdOutput)
                                    this.props.updateButtonToolbar('')
                        ...
                        if (this._validateSave()) {
                            this.props.validateOutput(objDataOutput.IdOrder).then(res => {
                                let { data } = res.data
                                if (data.length == 0) {
                                    this.props.getNumberAutoOutput().then(() => {
                                        this.props.addListProducts(ListProduct_temp_new)
                                            .then(res => {
                                                let { data } = res.data
                                                if (data && data.ErrorMessage) {
                                                    alert('Lỗi trong quá trình lưu dữ liệu.')
                                                } else {
                                                    this.props.addNewItemOutput(ListProduct_temp_new).then(() => {
                                                        //Cap nhat lai trong luong san pham bo
                                                        // $(`#toolbar_${valueTemp}`).css('pointer-events', 'auto');
                                                        this.child._addNotification(`Cập nhật thành công!!!`, 'success')
                                                        const { objDataOutput } = this.props.output
                                    ...
                                    let strIdOutPut = ''
                                    data.map(item => {
                                        strIdOutPut = strIdOutPut + item.IdOutput + ', '
                                    })
                                    // $(`#toolbar_${valueTemp}`).css('pointer-events', 'auto');
                                    this.child._addNotification(`Vui lòng xác nhận các phiếu xuất ${strIdOutPut}`, 'warning')
                                }
                            })
                        }
                        else {
                    ...
                    break;
                case "SAVE":
                    // $(`#toolbar_${'SAVE'}`).css('pointer-events', 'none');
                    if (status == '') {
                        isStatus = false
                        break
                    }
                        ...
                        this.props.updateAddListProducts(ListProduct_temp_new).then(res => {
                            ...
                            if (data && data.ErrorMessage) {
                                ...
                                this.props.updateItemOutput(ListProduct_temp_new).then(() => {
                                    ...
                                    // $(`#toolbar_${'SAVE'}`).css('pointer-events', 'auto');
                                    this.child._addNotification(`Cập nhật thành công!!!`, 'success')
                                    //Cap nhat lai trong luong san pham bo
                                    const { objDataOutput } = this.props.output
                                    this.props.UpdateProductWeightReal(objDataOutput.IdOutput)
                ...
                case "EDIT":
                    ...
                    break;
                case "CANCEL":
                    var r = confirm(`Bạn có muốn huỷ/thoát phiếu xuất này? `)
                    if (r == true) {
