...
class TabProduct extends React.Component {
    ...
    _onAddProductonRow(IdGroup) {
        ...
        console.log('listProductFind_temp>>>>>', listProductFind_temp);
    ...
    render() {
        ...
        let { list_products, listProductsSelected, isLoadingOutput, objDataOutput } = this.props.output
                        ...
                        <tbody >
                            {ListProductByOrderOutputSort && ListProductByOrderOutputSort.map((item, i) => {
                                let { IdOrder, IdProduct, IdProductParent, Color, Number, NumberTemp, stt, ProductsEachOutput,
                                    Weight, WeightAvgProduct, WeightAvg, WeightProduct, WeightCustom, IdOdd, sttOther, index, IdGroup, isExists, remark, IdGroupStt, url_image, org_price, price, bgcolor } = item
                                // set url image
                                let filename = url_image ? Config.API_URL_IMAGE + url_image : "images/image-not-found.jpg"
                                //console.log(isEditProducts)
                                return (
                                    <tr style={{ "backgroundColor": bgcolor }} id={`tr_c${sttOther}`} key={`data_${IdOrder}_${i}`} >
                                        {/* <td>
                                    {
                                                            ...
                                                            {list_bag.length > 0 && list_bag.map((item, i) => {
                                                                ...
                                                                return (
                                                                    <tr style={{ "color": "#fff" }} key={`dataDetail_${i}`}>
                                                                        <td style={{ "textAlign": "center","color": "#fff" }}>{IdBag || ''}</td>
                                                                    </tr>
                                                                )
                                                            })}
                                                        </tbody>
