...
class ListProductFormView extends React.Component {
    ...
    _updatePriceOrder(item) {
        var r = confirm(`Bạn có muốn cập nhật giá mới ?`);
        if (r == true) {
            this.props.updatePriceOrder(item.IdOrder).then(res => {
    ...
    render() {
        ...
        let listHeaderTable = [], pageRoute = ''
        if (this.props.routing && this.props.routing.locationBeforeTransitions.pathname) {
            pageRoute = this.props.routing.locationBeforeTransitions.pathname
        }
        if (pageRoute == 'order') {
            listHeaderTable = listHeaderTableOrder
        } else {
            listHeaderTable = listHeaderTableOrderMold
        }
        return (
            <div>
                    ...
                    <tbody>
                        {list_data && list_data.map((item, i) => {
                            let { IdOrder, IdCustomer, DayMake, checked,
                                StatusOrderName, StatusOrderBagName, StatusOrderTransferName, ProductsEachBag, StatusMoldName } = item
                            return (
                                <tr key={`data_${IdOrder}`} onDoubleClick={() => this._onClickRowDouble(item, !checked)}>
                                    ...
                                    {
                                        pageRoute != 'order' ?
                                            <td style={{ textAlign: "left" }}><button onClick={() => this._acceptOrderMold(item)}><i className="fa fa-check" aria-hidden="true"></i></button></td>
                                            : ''
                                    }
                                    ...
                                    <ButtonPermission type="DETAIL" key="DETAIL" nameBtn="DETAIL" icon={`fa fa-info-circle`} data={{ item: item, checked: checked ? checked : !checked }} parentObject={this} />
