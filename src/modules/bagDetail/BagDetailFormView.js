...
class BagDetailFormView extends React.Component {
    ...
    _onPhantom(type) {
        ...
        let { list_report_bag, imgCheck, imgUnCheck } = this.props.bagDetail
        let arr_data = [];
        let arr_data2 = [];
        let arr_data_pdf = [];
        let filename ;
        ...
        list_report_bag && list_report_bag.map((item, i) => {
            ...
            let { Id, IdProduct, QtyCreated, IdOrder, Size, Type, Casting, Stone,
                TypeGold, Gamma, Custom, Image, Remark, DateCreated, nameMX, nameLH, nameLV, IdOdd, type_bag, CountOfStone } = item.itemDetail
            ...
            console.log(list_value_color_header.length )
                            ...
                            table: {
                                ...
                                body: [
                                    ...
                                    [{
                                        ...
                                        {
                                            text: 'Hội: ' + (nameLH + ' / Xi: ' + nameMX || '') + ' / Gold: ' + (nameLV || '')
                                            ,height: 20
                                            ,fontSize: parseInt(CountOfStone) < 17 ? (Casting || '').length>130?8:9 : 8.5
                                        },
                                        // { ["a","b"]
    ...
    _parseListStone(get_list_stone_detail, list_stone_by_color) {
        ...
        arr_temp.map((item) => {
            let check = get_list_stone_detail.filter(x => x.Total == item.Total && x.issplit == 1)
            if (check.length > 0) {
                item.issplit = 1
            }
            return item
