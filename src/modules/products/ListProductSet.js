...
class ListProductSet extends React.Component {
    constructor() {
        super()
    }
                ...
                <table className="table table-striped">
                    ...
                    <tbody>
                        {listProductsSelected && listProductsSelected.map((item, i) => {
                            let { value, label, sl, Image, Weight, price,
                                WeightReal } = item
                            if (value && value.length > 0) {
                                let WeightTotal = Helper.roundNumber((WeightReal || Weight) || 0, 4)
                                return (
                                    <tr key={`data_${i}`}>
                                        ...
                                        <td style={{ "textAlign": "center" }}>{WeightTotal}</td>
...
export default ReactRedux.connect(mapStateToProps, mapDispatchToProps)(ListProductSet)
