...
class listReportFormView extends React.Component {
    ...
    _renderParams() {
        ...
        let arr_params = []
        listParams.map(item => {
            let { TYPE, VALUE, TEXT, FIELD } = item
            switch (TYPE) {
                case "STRING":
                    arr_params.push(
    ...
    render() {
        const { list_data, objData, listHeaderTable, listParams } = this.props.listReport
        const { code, name, group, store, orderby, orien, size, status } = objData
        return (
                                ...
                                <table className="table table-striped">
                                    ...
                                    <tbody>
                                        {list_data && list_data.map((item, i) => {
                                            let { ORDERBY, TITLE, checked } = item
                                            return (
                                ...
                                <div className="row">
                                    <div className="col-md-12">
                                        <div className="form-group">
                                            ...
                                            <button onClick={() => this._onExportPDF()} >Xuất pdf</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
