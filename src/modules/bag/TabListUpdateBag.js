...
class TabListUpdateBag extends React.Component {
    ...
    _onRemoveBag(item) {
        var r = confirm(`Bạn có muốn xoá bag này!`);
        if (r == true) {
                ...
                <table className="table table-striped">
                    ...
                    <tbody>
                        {ListBag.length > 0 && ListBag.map((item, i) => {
                            let { Id, IdOrder, IdProduct, QtyBag, checked, StatusName, StatusPrintName, StatusWeight, remark, created_by, created_date } = item
                            let colorStatus = ''
                            switch (StatusWeight) {
                            ...
                            return (
                                <tr key={`data_${Id}`} style={{ backgroundColor: `${colorStatus}` }} >
                                    ...
                                    <td>{Id}</td>
                                    <td>{moment(created_date).format('DD/MM/YYYY')}</td>
                                    <td>{IdOrder}</td>
                                    ...
                                    <ButtonPermission key="DELETE_BAG" type="DELETE_BAG" nameBtn="DELETE_BAG" icon={`fa fa-trash-o`} data={{ item: item }} parentObject={this} />
