    ...
    return (dispatch, getState) => {
        ...
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}output/get_list_product_by_order_full`, { params: { IdOutput: objDataOutput.IdOutput } })
                .then((response) => {
                    let { data } = response.data
                    let ListProductDefault = _.clone(ListProductByOrderOutput, true)
                    if (data.length > ListProductDefault.length) {
                        data.map((item, i) => {
                            if (i >= 50) {
                                item.index = i + 1
                                item.stt = item.IdProductParent + '_' + item.Color
                                item.sttOther = item.IdProduct + '_' + i
                                ListProductDefault.push(item)
                            }
                        })
                        dispatch({
                            type: GET_LIST_PRODUCT_BY_ORDER_IN_OUTPUT,
                            payload: {
                                ListProductByOrderOutput: ListProductDefault,
                                isLoadingOutput: false
                            }
                        })
                    } else {
...
export const getListDataOutPutBySearch = (value) => {
    return (dispatch, getState) => {
        ...
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}output/search`, { params: { page: pageParams.page, total: pageParams.total, key: value } })
                .then((response) => {
                    let { data } = response.data
                    let data_temp = data
...
export const printOuputDetail = (itemDetail, type) => {
    return (dispatch, getState) => {
        ...
        if (listId) {
            ...
            stringFile = 'Xuất hàng_' + itemDetail.IdOrder
        }
        ...
        return new Promise((resolve, reject) => {
            axios.post(`${Config.API_URL_USER}ouput/get_data_report_output`
                , { data: data, idreport: idreport })
                .then(res => {
                    let { data } = res
                    axios.post(`${Config.API_URL_USER}ouput/print_detail_output`,
                        { listProduct: data.data, Info: itemDetail, type: type },
                        {
                        ...
                        .then((response) => {
                            ...
                            link.href = url;
                            if (type == 'type3') {
                                link.setAttribute('download', `PackingList_${itemDetail.IdOrder}`); //or any other extension
                            }
                            else {
                                link.setAttribute('download', stringFile); //or any other extension
                            }
...
export const clickCheckRowOrder = (value, checked) => {
    return (dispatch, getState) => {
        ...
        setTimeout(() => {
            if (status == 'EDIT') {
                dispatch(getListProductByOrderInOutput()).then(() => {
...
export const addNewItemOutput = (data) => {
    ...
    return (dispatch, getState) => {
        ...
        data_temp = ListProduct_temp.map((item, i) => {
            ...
            item.IdGroupBy = IdProduct + IdProductParentColor + NumberTemp + Weight + WeightProduct + WeightCustom + price
            item.countItem = 1
            // cộng dồn số lượng
            if (status == 'EDIT') {
...
export const updateItemOutput = (ListProduct_temp) => {
    return (dispatch, getState) => {
        ...
        dispatch(UpdateProductWeightReal(objDataOutput.IdOutput))
    }
}
