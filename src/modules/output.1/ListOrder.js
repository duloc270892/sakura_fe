...
class ListOrder extends React.Component {
    ...
    _onClickRow(IdOrder, checked) {
        if (checked) {
            this.props.validateOutput(IdOrder).then(res => {
                let { data } = res.data
                if (data.length == 0) {
    ...
    componentDidMount() {
        ...
        // let that = this
        // let paddingTable = 215
        // let heightData = 900
        // $(window).scroll(function () {
        //     var wintop = $(window).scrollTop()
        //     let { pageLoad } = that.props.output
        //     let totalHeight = 215 + (heightData * pageLoad)
        //     if (wintop > totalHeight) {
        //         that.props.updateItemPageLoad(pageLoad + 1).then(() => {
        //             that.props.getListProductsByOrderOutputOnPage(pageLoad + 1)
        //         })
        //     }
        // });
    }
    ...
    _loadData() {
        this.props.getListDataOrderInOutPut().then(() => {
            console.log('fagta>>>>');
    ...
    render() {
        ...
        let { list_products, listProductsSelected, list_order_in_output, isShowProduct, isLoadingOutput } = this.props.output
            ...
            <div className="col-md-12">
                ...
                <div className="form-group">
                    <div style={{ width: "100%" }}>
                                    ...
                                    <tbody>
                                        {list_order_in_output && list_order_in_output.map((item, i) => {
                                            let { IdOrder, IdCustomer, DayMake, checked, TypeOrder,
                                                StatusOrderName, StatusOrderBagName, StatusOrderTransferName, ProductsEachBag, nameLV, nameLAI, Pricename } = item
                                            return (
                        ...
                        <div className="row">
                            <div className="col-md-12" id="aside-output">
                                <button onClick={() => this._onButtonAddProduct()}>Thêm sản phẩm</button>
                                <TabProduct heightProduct={heightList} key={'order'} />
                                <div className="loading-bar loadMoreGame hide"><span></span></div>
